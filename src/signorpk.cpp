...
bytes
SignorPK::sign(const bytes& plaintext, const privkey_type& privkey)
{
...
bytes
SignorPK::sign_detached(const bytes& plaintext, const privkey_type& privkey)
{
    ...
    if (crypto_sign_detached(signature.data(),
                             ...
                             privkey.data()) == -1)
        throw std::runtime_error{
            "sodium::SignorPK::sign_detached(): crypto_sign_detached() -1"
        };
...
bytes
SignorPK::verify(const bytes& plaintext_with_signature, const bytes& pubkey)
{
    ...
    if (plaintext_with_signature.size() < SIGNATURE_SIZE)
        throw std::runtime_error{
            "sodium::SignorPK::verify(): plaintext_with_signature too small "
            "for signature"
        };
