...
template<class BT>
BT
secretbox<BT>::encrypt(const BT& plaintext, const nonce_type& nonce)
{
    ...
    crypto_secretbox_easy(
      reinterpret_cast<unsigned char*>(ciphertext_with_mac.data()),
                       ...
                       const nonce_type& nonce)
{
    ...
    if (ciphertext_with_mac.size() != plaintext.size() + MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::encrypt() wrong ciphertext_with_mac size"
        };
...
template<class BT>
BT
secretbox<BT>::encrypt(const BT& plaintext, const nonce_type& nonce, BT& mac)
{
    // some sanity checks before we get started
    if (mac.size() != MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::encrypt(detached) wrong mac size"
        };
                       ...
                       BT& mac)
{
    ...
    if (ciphertext.size() != plaintext.size())
        throw std::runtime_error{
            "sodium::secretbox::encrypt(detached) wrong ciphertext size"
        };
    if (mac.size() != MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::encrypt(detached) wrong mac size"
...
template<class BT>
BT
secretbox<BT>::decrypt(const BT& ciphertext_with_mac, const nonce_type& nonce)
{
    // some sanity checks before we get started
    if (ciphertext_with_mac.size() < MACSIZE)
        throw std::runtime_error{ "sodium::secretbox::decrypt(combined) "
                                  "ciphertext_with_mac too small for mac" };
    ...
    if (crypto_secretbox_open_easy(
          ...
          key_.data()) != 0)
        throw std::runtime_error{
            "sodium::secretbox::decrypt(combined) can't decrypt"
        };
                       ...
                       const nonce_type& nonce)
{
    ...
    if (decrypted.size() != ciphertext_with_mac.size() - MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::decrypt(combined) decrypted wrong size"
        };
                       ...
                       const BT& mac)
{
    ...
    if (mac.size() != MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::decrypt(detached) wrong mac size"
        };
    ...
    if (crypto_secretbox_open_detached(
          ...
          key_.data()) != 0)
        throw std::runtime_error{
            "sodium::secretbox::decrypt(detached) can't decrypt"
        };
                       ...
                       const BT& mac)
{
    ...
    if (decrypted.size() != ciphertext.size())
        throw std::runtime_error{
            "sodium::secretbox::decrypt(detached) wrong decrypted size"
        };
    if (mac.size() != MACSIZE)
        throw std::runtime_error{
            "sodium::secretbox::decrypt(detached) wrong mac size"
