...
class secretbox_encrypt_filter : public io::aggregate_filter<char>
{
     ...
     *     namespace io = boost::iostreams;
     *
     *     secretbox_encrypt_filter::key_type      key;      // Create a random
     *key secretbox_encrypt_filter::nonce_type    nonce;    // create random
     *nonce secretbox crypt{ std::move(key) }; secretbox_encrypt_filter
     *encrypt_filter {std::move(crypt), nonce};  // create a secretbox filter
     *
     *     chars ciphertext(ensecretbox_encrypt_filter::MACSIZE +
     *plainblob.size());
     *
     ...
     *encrypt_filter {std::move(crypt), nonce};  // create a secretbox filter
     *
     *     chars ciphertext ( secretbox_encrypt_filter::MACSIZE +
     *plaintext.size() ); io::array_source        source {plainblob.data(),
     *plainblob.size()}; io::filtering_istream   is     {};
  ...
  public:
    ...
    secretbox_encrypt_filter(const secretbox<vector_type>& secretbox,
                             const nonce_type& nonce)
      : secretbox_{ secretbox }
      , nonce_{ nonce }
    {}

    secretbox_encrypt_filter(secretbox<vector_type>&& secretbox,
                             const nonce_type& nonce)
...
#ifndef NDEBUG
        std::cerr << "secretbox_encrypt_filter::do_filter() called"
                  << std::endl;
#endif // ! NDEBUG
