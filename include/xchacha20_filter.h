// xchacha20_filter.h -- Boost.Iostreams filter for XChaCha20 stream cipher
//
// ISC License
//
...
{
    /**
     * xchacha20_symmetric_filter is a SymmetricFilter model that applies
     * the XChaCha20 stream cipher.  This model can be turned into a
     * DualUse filter using the xchacha20_filter class below (see there
  ...
  public:
    ...
    /**
     * Construct a SymmetricFilter model for the XChaCha20 stream cipher.
     *
     * Parameters:
     *   key   : the secret key used to encrypt/decrypt.
     *   nonce : a public nonce.
     *
     * Even though we supply a nonce, xchacha20_symmetric_filter uses a
    ...
    xchacha20_symmetric_filter(const key_type& key, const nonce_type& nonce)
      : key_{ key }
      , nonce_{ nonce }
      , initptr_{ nullptr }
    {}
     ...
     * XChaCha20 algorithm.  Update i1 and o1 after filtering.
     *
...
#endif // ! NDEBUG
    {
        ...
        if (crypto_stream_xchacha20_xor_ic(
              ...
              ic,
              key_.data()) == -1)
            throw std::runtime_error{ "sodium::xchacha20_filter::filter() "
                                      "crypto_stream_xchacha20_xor_ic() -1" };
        ...
        if (i1 != i2 && o1 != o2)
            throw std::runtime_error{
                "sodium::xchacha20_filter::filter() postcondition failed"
            };
     ...
     * XChaCha20 counter indirectly to 0 by resetting initptr_.
     **/

    void close()
    {
#ifndef NDEBUG
        std::cerr << "xchacha20_symmetric_filter::close() called" << std::endl;
#endif // ! NDEBUG
...
}; // xchacha20_symmetric_filter

// Turn xchacha20_symmetric_filter into a DualUse filter class:

class xchacha20_filter : public io::symmetric_filter<xchacha20_symmetric_filter>
{
    /**
     * xchacha20_filter is a DualUseFilter that performs
     ...
     *
     * See also: xchacha20_symmetric_filter documentation above.
     *
     * Use xchacha20_filter as a DualUseFilter like this:
     *
  ...
  private:
    typedef io::symmetric_filter<xchacha20_symmetric_filter> base_type;
    typedef xchacha20_symmetric_filter symmetric_filter_type;
  ...
  public:
    ...
    /**
     * xchacha20_filter constructs a DualUseFilter out of the
     * xchacha20_symmetric_filter SymmetricFilter model to perform
     * encryption/decryption according to the XChaCha20 algorithm.
     *
     * Parameters:
     *   buffer_size: proceed encryption/decryption in blocks of so many bytes.
     *   key        : secret key used to encrypt/decrypt data with XChaCha20
     *   nonce      : public nonce used to encrypt/decrypt.
     *
     * Note that to facilitate computations, buffer_size will always be
     * rounded up to the next xchacha20_symmetric_filter::BLOCKSIZE
     * boundary, i.e. when BLOCKSIZE is 64 bytes: 20 -> 64, 64 -> 64,
     * 100 -> 128, and so on.
     **/

    xchacha20_filter(std::streamsize buffer_size,
                     const key_type& key,
                     const nonce_type& nonce)
      : base_type(round_up_to_xchacha20_blocksize(buffer_size), key, nonce)
    {}

  private:
    std::streamsize round_up_to_xchacha20_blocksize(std::streamsize n) const
    {
...
/**
 * Turn xchacha20_filter into a Pipable filter, so in can appear
 * in a pipeline.
 *
 * Usage:
