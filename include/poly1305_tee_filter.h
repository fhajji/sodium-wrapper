 ...
 *   io::filtering_ostream os(filter1 | filter2 | poly1305_filter | filter3 |
 *somesink);
 *
 * Usage (example):
 *
  ...
  public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        throw std::runtime_error{
            "sodium::poly1305_tee_filter::read() called. FIXME!"
        };
    ...
    void close(Next&, BOOST_IOS::openmode)
    {
        ...
        (void)
#endif // ! NDEBUG
          boost::iostreams::write(this->component(), out, MACSIZE);

#ifndef NDEBUG
        mac_type out_as_mac_type_var{ out, out + MACSIZE };
        std::string out_as_string{ sodium::bin2hex<mac_type>(
          out_as_mac_type_var) };
...
template<typename Sink>
poly1305_tee_filter<Sink>
poly1305_tee(Sink& snk, const typename poly1305_tee_filter<Sink>::key_type& key)
{
 ...
 * using poly1305_to_vector_type = poly1305_tee_device<io::file_sink,
 *vector_sink>;
 *
 * poly1305_to_vector_type::key_type key; // generate a random key for Poly1305
 *
...
class poly1305_tee_device
{
  public:
    ...
    poly1305_tee_device(device_param device,
                        sink_param sink,
                        const key_type& key)
      : dev_(device)
      , sink_(sink)
    ...
    std::streamsize read(char_type* s, std::streamsize n)
    {
        ...
        throw std::runtime_error{
            "sodium::poly1305_tee_device::read() called. FIXME!"
        };
    ...
    void close()
    {
        // before closing, send the computed Poly1305 MAC:
        char_type
          out[MACSIZE]; // XXX: actually, char_type of sink_, not of dev_
        crypto_onetimeauth_final(&state_,
                                 reinterpret_cast<unsigned char*>(out));
        ...
        (void)
#endif // ! NDEBUG
          boost::iostreams::write(sink_, out, MACSIZE);
        ...
        BOOST_ASSERT(result ==
                     MACSIZE); // sanity check: we didn't lose anything
