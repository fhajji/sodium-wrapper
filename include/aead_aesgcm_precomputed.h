// aead_aesgcm_precomputed.h -- AES-GCM AEAD construction with pre-computed
// keys.
//
// ISC License
//
...
{
  public:
    constexpr static const char* construction_name = "aesgcm_precomputed";
    ...
    static int init_ctx(ctx_type* ctx, unsigned const char* key)
    {
        return crypto_aead_aes256gcm_beforenm(ctx, key);
    }
                       ...
                       const ctx_type* ctx)
    {
        return crypto_aead_aes256gcm_encrypt_afternm(
          c, clen, m, mlen, ad, adlen, nsec, npub, ctx);
    };
                       ...
                       const ctx_type* ctx)
    {
        return crypto_aead_aes256gcm_decrypt_afternm(
          m, mlen, nsec, c, clen, ad, adlen, npub, ctx);
    };
                                ...
                                const ctx_type* ctx)
    {
        return crypto_aead_aes256gcm_encrypt_detached_afternm(
          c, mac, maclen_p, m, mlen, ad, adlen, nsec, npub, ctx);
    };
                                ...
                                const ctx_type* ctx)
    {
        return crypto_aead_aes256gcm_decrypt_detached_afternm(
          m, nsec, c, clen, mac, ad, adlen, npub, ctx);
    };
