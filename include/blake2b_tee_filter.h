 ...
 *   io::filtering_ostream os(filter1 | filter2 | blake2b_filter | filter3 |
 *somesink);
 *
 * Usage (example):
 *
  ...
  public:
      ...
      , hashsize_{ hashsize }
    {
        ...
        if (key.size() != 0 && key.size() < KEYSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filter() key size too small" };
        if (key.size() != 0 && key.size() > KEYSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filter() key size too big" };

        if (hashsize < HASHSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filter() hash size too small" };
        if (hashsize > HASHSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filter() hash size too big" };
      ...
      , hashsize_{ hashsize }
    {
        ...
        if (hashsize < HASHSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filter(keyless) hash size too small" };
        if (hashsize > HASHSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_filter::blake2b_tee_"
                                      "filterkeyless) hash size too big" };
...
#ifndef NDEBUG
        std::cerr
          << "sodium::blake2b_tee_filter::blake2b_tee_filter(keyless) called"
          << std::endl;
#endif // ! NDEBUG
    }
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        throw std::runtime_error{
            "sodium::blake2b_tee_filter::read() called. FIXME!"
        };
        ...
        if (result != -1) {
            ...
            std::cerr << "  [s=" << s_as_string << "]" << std::endl;
        }
#endif // ! NDEBUG
        ...
        //
        //   // std::streamsize result2 = iostreams::write(this->component(), s,
        //   result);
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        ...
        crypto_generichash_update(
          &state_, reinterpret_cast<const unsigned char*>(s), result);

        // Don't write anything yet to the second sink, because we're not
        // done yet computing the BLAKE2b MAC:

        // std::streamsize result2 = iostreams::write(this->component(), s,
        // result); (void) result2; // Suppress 'unused variable' warning.
        //  BOOST_ASSERT(result == result2);
    ...
    void close(Next&, BOOST_IOS::openmode)
    {
        ...
        crypto_generichash_final(
          &state_, reinterpret_cast<unsigned char*>(out), hashsize_);
        ...
        (void)
#endif // ! NDEBUG
          boost::iostreams::write(this->component(), out, hashsize_);

#ifndef NDEBUG
        hash_type out_as_hash_type_var{ out, out + hashsize_ };
        std::string out_as_hex_string{ sodium::bin2hex<hash_type>(
          out_as_hash_type_var) };
        std::cerr << "sodium::blake2b_tee_filter::close() called "
                  << "[result=" << result << "], "
                  << "[hashsize=" << hashsize_ << "]" << '\n'
                  << "  [out=" << out_as_hex_string << "]" << std::endl;
#endif // ! NDEBUG
    ...
    bool flush(Sink& snk)
    {
        ...
        bool r2 =
          boost::iostreams::flush(this->component()); // actually a NO-OP
...
template<typename Sink>
blake2b_tee_filter<Sink>
blake2b_tee(Sink& snk,
 ...
 * using chars = sodium::chars;
 *
 * using hash_array_type = typename
 *blake2b_tee_filter<io::null_sink>::hash_type; using vector_sink    =
 *io::back_insert_device<hash_array_type>;
 *
 * // an output filter that outputs to io::file_sink and tee-s to vector_sink
 * using blake2b_to_vector_type = blake2b_tee_device<io::file_sink,
 *vector_sink>;
 *
...
class blake2b_tee_device
{
  public:
    ...
    BOOST_STATIC_ASSERT(
      (boost::is_same<char_type,
                      BOOST_DEDUCED_TYPENAME char_type_of<Sink>::type>::value));
    BOOST_STATIC_ASSERT(
      (boost::is_convertible<
    ...
    blake2b_tee_device(device_param device,
                       sink_param sink,
                       const key_type& key,
                       const std::size_t hashsize)
      : dev_(device)
      ...
      , hashsize_{ hashsize }
    {
        // Some sanity checks first regarding the key and desired size
        if (key.size() != 0 && key.size() < KEYSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device() key size too small" };
        if (key.size() != 0 && key.size() > KEYSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device() key size too big" };

        if (hashsize < HASHSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device() hash size too small" };
        if (hashsize > HASHSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device() hash size too big" };
    ...
    explicit blake2b_tee_device(device_param device,
                                sink_param sink,
                                const std::size_t hashsize = HASHSIZE)
      : dev_(device)
      , sink_(sink)
      ...
      , hashsize_{ hashsize }
    {
        ...
        if (hashsize < HASHSIZE_MIN)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device(keyless) hash size too small" };
        if (hashsize > HASHSIZE_MAX)
            throw std::runtime_error{ "sodium::blake2b_tee_device::blake2b_tee_"
                                      "device(keyless) hash size too big" };
...
#ifndef NDEBUG
        std::cerr
          << "sodium::blake2b_tee_device::blake2b_tee_device(keyless) called"
          << std::endl;
#endif // ! NDEBUG
    }
    ...
    std::streamsize read(char_type* s, std::streamsize n)
    {
        ...
        throw std::runtime_error{
            "sodium::blake2b_tee_device::read() called. FIXME!"
        };
        ...
        //   // not done yet computing the BLAKE2b hash.
        //   //
        //   // // std::streamsize result2 = iostreams::write(sink_, s,
        //   result1);
    ...
    std::streamsize write(const char_type* s, std::streamsize n)
    {
        ...
        // Update the BLAKE2b state with the chunk we've got:
        crypto_generichash_update(
          &state_, reinterpret_cast<const unsigned char*>(s), result1);
    ...
    void close()
    {
        ...
        auto out = new char_type[hashsize_]; // XXX: actually, char_type of
                                             // sink_, not of dev_
        crypto_generichash_final(
          &state_, reinterpret_cast<unsigned char*>(out), hashsize_);
        ...
        (void)
#endif // ! NDEBUG
          boost::iostreams::write(sink_, out, hashsize_);

#ifndef NDEBUG
        hash_type out_as_hash_type_var{ out, out + hashsize_ };
        std::string out_as_string{ sodium::bin2hex<hash_type>(
          out_as_hash_type_var) };
        std::cerr << "sodium::blake2b_tee_device::close() called "
                  << "[result=" << result << "] "
                  << "[hashsize=" << hashsize_ << "]" << '\n'
                  << "  [out=" << out_as_string << "]" << std::endl;
#endif // ! NDEBUG
