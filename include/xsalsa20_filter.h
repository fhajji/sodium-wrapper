// xsalsa20_filter.h -- Boost.Iostreams filter for XSalsa20 stream cipher
//
// ISC License
//
...
class xsalsa20_symmetric_filter
{
    /**
     * xsalsa20_symmetric_filter is a SymmetricFilter model that applies
     * the XSalsa20 stream cipher.  This model can be turned into a
     * DualUse filter using the xsalsa20_filter class below (see there
     * how to use it in user code).
     **/

  public:
    static constexpr std::size_t KEYSIZE = sodium::KEYSIZE_XSALSA20;
    static constexpr std::size_t NONCESIZE = sodium::NONCESIZE_XSALSA20;
    ...
    /**
     * Construct a SymmetricFilter model for the XSalsa20 stream cipher.
     *
     * Parameters:
     *   key   : the secret key used to encrypt/decrypt.
     *   nonce : a public nonce.
     *
     * Even though we supply a nonce, xsalsa20_symmetric_filter uses a
    ...
    xsalsa20_symmetric_filter(const key_type& key, const nonce_type& nonce)
      : key_{ key }
      , nonce_{ nonce }
      , initptr_{ nullptr }
    {}
     ...
     * XSalsa20 algorithm.  Update i1 and o1 after filtering.
     *
    ...
    bool filter(const char_type*& i1,
                const char_type* i2,
                char_type*& o1,
                char_type* o2,
#ifndef NDEBUG
...
#endif // ! NDEBUG
    {
        ...
        auto mlen = static_cast<unsigned long long>(
          std::min<std::ptrdiff_t>(i2 - i1, o2 - o1));
        ...
        if (crypto_stream_xsalsa20_xor_ic(
              reinterpret_cast<unsigned char*>(o1),
              reinterpret_cast<const unsigned char*>(i1),
              mlen,
              nonce_.data(),
              ic,
              key_.data()) == -1)
            throw std::runtime_error{ "sodium::xsalsa20_filter::filter() "
                                      "crypto_stream_xsalsa20_xor_ic() -1" };

#ifndef NDEBUG
        std::cerr << "xsalsa20_symmetric_filter::filter("
                  << static_cast<const void*>(i1) << ","
                  << static_cast<const void*>(i2) << ","
                  << static_cast<const void*>(o1) << ","
                  << static_cast<const void*>(o2) << "," << flush << ") called"
                  << '\n'
                  << "  [mlen=" << mlen << "]" << '\n'
                  << "  [[i1,i1+mlen)={" << std::string(i1, i1 + mlen) << "}"
                  << '\n'
                  << "  [[o1,o1+mlen)={" << std::string(o1, o1 + mlen) << "}"
                  << "  [ic]=" << ic << '\n'
                  << std::endl;
#endif // ! NDEBUG
        ...
        if (i1 != i2 && o1 != o2)
            throw std::runtime_error{
                "sodium::xsalsa20_filter::filter() postcondition failed"
            };
     ...
     * XSalsa20 counter indirectly to 0 by resetting initptr_.
     **/

    void close()
    {
#ifndef NDEBUG
        std::cerr << "xsalsa20_symmetric_filter::close() called" << std::endl;
#endif // ! NDEBUG
...
}; // xsalsa20_symmetric_filter

// Turn xsalsa20_symmetric_filter into a DualUse filter class:

class xsalsa20_filter : public io::symmetric_filter<xsalsa20_symmetric_filter>
{
    /**
     * xsalsa20_filter is a DualUseFilter that performs
     ...
     *
     * See also: xsalsa20_symmetric_filter documentation above.
     *
     * Use xsalsa20_filter as a DualUseFilter like this:
     *
  ...
  private:
    typedef io::symmetric_filter<xsalsa20_symmetric_filter> base_type;
    typedef xsalsa20_symmetric_filter symmetric_filter_type;
  ...
  public:
    ...
    /**
     * xsalsa20_filter constructs a DualUseFilter out of the
     * xsalsa20_symmetric_filter SymmetricFilter model to perform
     ...
     *   key        : secret key used to encrypt/decrypt data with XSalsa20
     *   nonce      : public nonce used to encrypt/decrypt.
     *
     * Note that to facilitate computations, buffer_size will always be
     * rounded up to the next xsalsa20_symmetric_filter::BLOCKSIZE
     * boundary, i.e. when BLOCKSIZE is 64 bytes: 20 -> 64, 64 -> 64,
     * 100 -> 128, and so on.
     **/

    xsalsa20_filter(std::streamsize buffer_size,
                    const key_type& key,
                    const nonce_type& nonce)
      : base_type(round_up_to_xsalsa20_blocksize(buffer_size), key, nonce)
    {}

  private:
    std::streamsize round_up_to_xsalsa20_blocksize(std::streamsize n) const
    {
        return ((n / symmetric_filter_type::BLOCKSIZE) + 1) *
               symmetric_filter_type::BLOCKSIZE;
    }
};

/**
 * Turn xsalsa20_filter into a Pipable filter, so in can appear
 * in a pipeline.
 *
 * Usage:
...
BOOST_IOSTREAMS_PIPABLE(xsalsa20_filter, 0)
