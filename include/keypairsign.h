...
class keypairsign
{
  ...
  public:
     ...
     *
     * The created keypairsign contains a public key with KEYSIZE_PUBLIC_KEY
     *bytes, and a private key with KEYSIZE_PRIVATE_KEY bytes. Both keys are
     *related and must be used together.
     *
      ...
      , private_key_(false)
    {
        if (crypto_sign_keypair(
              ...
              private_key_.setdata()) == -1)
            throw std::runtime_error{
                "sodium::keypairsign::keypairsign() crypto_sign_keypair() -1"
            };
      ...
      , private_key_(false)
    {
        if (seed.size() != KEYSIZE_SEEDBYTES)
            throw std::runtime_error{
                "sodium::keypairsign::keypairsign(seed) wrong seed size"
            };
        ...
        if (crypto_sign_seed_keypair(
            ...
            throw std::runtime_error{ "sodium::keypairsign::keypairsign(seed..."
      ...
      , private_key_(false)
    {
        if (private_key_size != KEYSIZE_PRIVATE_KEY)
            throw std::runtime_error{
                "sodium::keypairsign::keypairsign(private_key_data, "
                "private_key_size) wrong private_key_size"
            };
    ...
    seed_type seed()
    {
        ...
        if (crypto_sign_ed25519_sk_to_seed(
              reinterpret_cast<unsigned char*>(the_seed.data()),
              private_key_.data()) == -1)
            throw std::runtime_error{ "sodium::keypairsign::seed() "
...
bool
operator==(const sodium::keypairsign<PK1>& kp1,
