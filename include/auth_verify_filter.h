// auth_verify_filter.h -- Boost.Iostreams MAC verifying filter for Secret Key
// Authentication (MAC)
//
// ISC License
//
...
class auth_verify_filter : public io::aggregate_filter<char>
{
     ...
     *dog"}; chars       plainblob {plaintext.cbegin(), plaintext.cend()}; chars
     *mac = ... // computed earlier auth_verify_filter::key_type key { ... }; //
     *same as key used earlier
     *
     ...
     *     auth_verify_filter verify_filter {std::move {auth}, mac}; // create a
     *MAC verifier filter
     *
     ...
     *
     *     authenticator<chars> auth { std::move(key) };
     *     auth_verify_filter   verify_filter {std::move(auth), mac}; // create
     *a MAC verifier filter
     *
  ...
  public:
    ...
    auth_verify_filter(const authenticator<vector_type>& auth,
                       const vector_type& mac)
      : auth_{ auth }
      , mac_{ mac }
    {
        if (mac.size() != MACSIZE)
            throw std::runtime_error{ "sodium::auth_verify_filter::auth_verify_"
                                      "filter() wrong MAC size" };
    }

    auth_verify_filter(authenticator<vector_type>&& auth,
                       const vector_type& mac)
      : auth_{ std::move(auth) }
      , mac_{ mac }
    {
    ...
    virtual void do_filter(const vector_type& src, vector_type& dest)
    {
        ...
        if (mac_.size() != MACSIZE)
            throw std::runtime_error{
                "sodium::auth_verify_filter::do_filter() mac wrong size"
            };
