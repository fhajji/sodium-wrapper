// auth_mac_filter.h -- Boost.Iostreams MAC generating filter for Secret Key
// Authentication (MAC)
//
// ISC License
//
...
class auth_mac_filter : public io::aggregate_filter<char>
{
     ...
     *     namespace io = boost::iostreams;
     *
     *     auth_mac_filter::key_type key;                   // create a random
     *key authenticator<chars>      auth {std::move(key)}; // create an
     *authenticator auth_mac_filter           mac_filter {std::move(auth)}; //
     *create a MAC creator filter
     *
