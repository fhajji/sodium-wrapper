...
test_of_correctness(const std::string &plaintext)
{
  ...
  BT decrypted_by_bob_from_someone =
...
falsify_seal(const std::string &plaintext)
{
  ...
  // encrypt to self
  BT ciphertext = sb.encrypt(plainblob,
				 keypair_alice);
  ...
  try {
    BT decrypted = sb.decrypt(ciphertext,
				  keypair_alice);
  }
...
BOOST_AUTO_TEST_CASE( sodium_sealedbox_test_detect_wrong_recipient_fulltext_bytes )
{
...
BOOST_AUTO_TEST_CASE( sodium_sealedbox_test_detect_wrong_recipient_empty_text_bytes)
{
  std::string plaintext {};
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_encrypt_to_self_bytes_protected)
{
	...
	// encrypt to self
	sodium::bytes_protected ciphertext = sb.encrypt(plainblob,
		keypair_alice);

	BOOST_CHECK_EQUAL(ciphertext.size(),
		plainblob.size() + box_seal<sodium::bytes_protected>::SEALSIZE);

	sodium::bytes_protected decrypted = sb.decrypt(ciphertext,
		keypair_alice);
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_detect_wrong_recipient_fulltext_bytes_protected)
{
	std::string plaintext{ "Hi Bob, this is your secret admirer!" };
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_detect_wrong_recipient_empty_text_bytes_protected)
{
	std::string plaintext{};
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_falsify_seal_fulltext_bytes_protected)
{
	std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_encrypt_to_self_chars)
{
	...
	// encrypt to self
	sodium::chars ciphertext = sb.encrypt(plainblob,
		keypair_alice);

	BOOST_CHECK_EQUAL(ciphertext.size(),
		plainblob.size() + box_seal<sodium::chars>::SEALSIZE);

	sodium::chars decrypted = sb.decrypt(ciphertext,
		keypair_alice);
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_detect_wrong_recipient_fulltext_chars)
{
	std::string plaintext{ "Hi Bob, this is your secret admirer!" };
...
BOOST_AUTO_TEST_CASE(sodium_sealedbox_test_detect_wrong_recipient_empty_text_chars)
{
	std::string plaintext{};
