...
test_keyless_hashing(const std::string& plaintext)
{
    ...
    try {
        bytes outHash1 = hashor.hash(
          plainblob /* , hashor_generic_keyless::HASHSIZE */); // keyless
...
BOOST_AUTO_TEST_CASE(sodium_hashor_generic_keyless_test_min_hash_size)
{
    ...
    BOOST_CHECK(
      test_hash_size(plaintext, hashor_generic_keyless::HASHSIZE_MIN));
}
...
BOOST_AUTO_TEST_CASE(sodium_hashor_generic_keyless_test_max_hash_size)
{
    ...
    BOOST_CHECK(
      test_hash_size(plaintext, hashor_generic_keyless::HASHSIZE_MAX));
}
...
BOOST_AUTO_TEST_CASE(sodium_hashor_generic_keyless_test_hash_size_too_small)
{
    ...
    BOOST_CHECK(
      !test_hash_size(plaintext, hashor_generic_keyless::HASHSIZE_MIN - 1));
}
...
BOOST_AUTO_TEST_CASE(sodium_hashor_generic_keyless_test_hash_size_too_big)
{
    ...
    BOOST_CHECK(
      !test_hash_size(plaintext, hashor_generic_keyless::HASHSIZE_MAX + 1));
}
...
BOOST_AUTO_TEST_CASE(sodium_hashor_generic_keyless_test_falsify_plaintext)
{
    ...
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    hashor_generic_keyless::bytes_type plainblob{ plaintext.cbegin(),
                                                  plaintext.cend() };
    hashor_generic_keyless::bytes_type falsified{ plainblob };
    ++falsified[0];

    bytes hash1 =
      hashor.hash(plainblob /* , hashor_generic_keyless::HASHSIZE */);
    bytes hash2 =
      hashor.hash(falsified /* , hashor_generic_keyless::HASHSIZE */);
