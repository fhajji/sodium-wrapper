...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_classic_compare_timing)
{
    ...
    auto time_classic_compare_equal =
    ...
    auto time_classic_compare_unequal =
    ...
    oss << "classical compare b1==b1: " << time_classic_compare_equal.count()
        << " milliseconds."
        << "\n";
    oss << "classical compare b1!=b2: " << time_classic_compare_unequal.count()
        << " milliseconds.";
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_sodium_compare_timing)
{
    ...
    auto t1 = std::chrono::system_clock::now();
    auto time_sodium_compare_equal =
      std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0);
    ...
    auto t3 = std::chrono::system_clock::now();
    auto time_sodium_compare_unequal =
      std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t2);
    ...
    std::ostringstream oss;
    oss << "sodium compare b1==b1: " << time_sodium_compare_equal.count()
        << " milliseconds."
        << "\n";
    oss << "sodium compare b1!=b2: " << time_sodium_compare_unequal.count()
        << " milliseconds.";
    BOOST_TEST_MESSAGE(oss.str());
}
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_sodium_is_zero_timing_all_zeroes)
{
    ...
    std::ostringstream oss;
    oss << "is_zero_naive(==0)   " << time_is_zero_naive_true.count()
        << " milliseconds."
        << "\n";
    oss << "sodium::is_zero(==0) " << time_sodium_is_zero_true.count()
        << " milliseconds.";
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_sodium_is_zero_timing_not_all_zeroes)
{
    ...
    for (int i = 0; i != TEST_COUNT_IS_ZERO; ++i)
        static_cast<void>(sodium::is_zero(b));
    auto t1 = std::chrono::system_clock::now();
    auto time_sodium_is_zero_false =
      std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0);
    ...
    std::ostringstream oss;
    oss << "is_zero_naive(!=0)   " << time_is_zero_naive_false.count()
        << " milliseconds."
        << "\n";
    oss << "sodium::is_zero(!=0) " << time_sodium_is_zero_false.count()
        << " milliseconds."
        << "\n";
...
BOOST_AUTO_TEST_CASE(
  sodium_test_helpers_bin2hex_return_string_protected_clearmem)
{
    std::string in1{ "0123456789" };
    sodium::bytes b1{ in1.cbegin(), in1.cend() };

    // returns a sodium::string_protected
    auto hexb1{ sodium::bin2hex<sodium::bytes, sodium::string_protected>(
      b1, true) };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_helpers_bin2hex_key_return_string_protected_clearmem)
{
    sodium::key<sodium::KEYSIZE_SECRETBOX> some_key; // random values

    auto hex_some_key{ sodium::bin2hex<sodium::key<sodium::KEYSIZE_SECRETBOX>,
                                       sodium::string_protected>(some_key,
                                                                 true) };
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_full)
{
    ...
    // selects sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
    // sodium::bytes>()
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_return_string_protected)
{
    ...
    auto base64b1{ sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
                                      sodium::bytes,
                                      sodium::string_protected>(b1) };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_helpers_bin2base64_return_string_protected_clearmem)
{
    std::string in1{ "subjects?_d" };
    sodium::bytes b1{ in1.cbegin(), in1.cend() };
    ...
    auto base64b1{ sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
                                      sodium::bytes,
                                      sodium::string_protected>(b1, true) };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_helpers_bin2base64_full_bytes_protected_clearmem)
{
    std::string in1{ "subjects?_d" };
    sodium::bytes_protected b1{ in1.cbegin(), in1.cend() };

    // selects sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
    // sodium::bytes_protected>(BT, bool)
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_key)
{
    ...
    // selects sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
    // sodium::key<sodium::KEYSIZE_SECRETBOX>() CAREFUL: temp buffer allocated
    // on the heap with key material in base64 NOT zeroed.
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_key_clearmem)
{
    ...
    // selects sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL,
    // sodium::key<sodium::KEYSIZE_SECRETBOX>(BT, bool) CAREFUL: though the temp
    // buffer on the heap with key material in base64 is zeroed, it was
    // accessible for a small time window.
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_full_nopadding)
{
    ...
    // selects sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL_NO_PADDING,
    // sodium::bytes>
    std::string base64b1{
        sodium::bin2base64<sodium_base64_VARIANT_ORIGINAL_NO_PADDING>(b1)
    };
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_full_urlsafe)
{
    ...
    std::string base64b1{ sodium::bin2base64<sodium_base64_VARIANT_URLSAFE>(
      b1) };
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_bin2base64_full_urlsafe_nopadding)
{
    ...
    // selects sodium::bin2base64<sodium_base64_VARIANT_URLSAFE_NO_PADDING,
    // sodium::bytes>
    std::string base64b1{
        sodium::bin2base64<sodium_base64_VARIANT_URLSAFE_NO_PADDING>(b1)
    };
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_full)
{
    ...
    // selects sodium::base642bin<sodium_base64_VARIANT_ORIGINAL, std::string,
    // sodium::bytes>()
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_chars)
{
    ...
    auto bin = sodium::base642bin<sodium_base64_VARIANT_ORIGINAL,
                                  std::string,
                                  sodium::chars>(in1);
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_bytes_protected)
{
    ...
    std::string result_as_string{ "subjects?_d" };
    sodium::bytes_protected result{ result_as_string.cbegin(),
                                    result_as_string.cend() };
    BOOST_CHECK(bin == result);
}

BOOST_AUTO_TEST_CASE(
  sodium_test_helpers_base642bin_string_protected_bytes_protected)
{
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_full_variant2)
{
    ...
    // selects sodium::base642bin<sodium_base64_VARIANT_ORIGINAL_NO_PADDING,
    // std::string, sodium::bytes>()
    auto bin =
      sodium::base642bin<sodium_base64_VARIANT_ORIGINAL_NO_PADDING>(in1);
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_full_variant3)
{
    ...
    // selects sodium::base642bin<sodium_base64_VARIANT_URLSAFE, std::string,
    // sodium::bytes>()
...
BOOST_AUTO_TEST_CASE(sodium_test_helpers_base642bin_full_variant4)
{
    ...
    // selects sodium::base642bin<sodium_base64_VARIANT_URLSAFE_NO_PADDING,
    // std::string, sodium::bytes>()
    auto bin =
      sodium::base642bin<sodium_base64_VARIANT_URLSAFE_NO_PADDING>(in1);
