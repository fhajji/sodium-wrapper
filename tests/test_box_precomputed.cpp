...
test_of_correctness(const std::string& plaintext)
{
    ...
    BT ciphertext_from_alice_to_bob = sc_alice.encrypt(plainblob, nonce);
...
test_of_correctness_detached(const std::string& plaintext)
{
    ...
    BT ciphertext_from_alice_to_bob = sc_alice.encrypt(plainblob, nonce, mac);
...
template<typename BT = bytes>
bool
destroy_shared_key_then_decrypt(
  const BT& ciphertext,
...
template<typename BT = bytes>
bool
destroy_shared_key_then_decrypt_detached(
  const BT& ciphertext,
  const keypair<BT>& keypair,
  const typename box_precomputed<BT>::nonce_type& nonce,
  const BT& mac)
...
time_encrypt(const unsigned long nr_of_messages)
{
    ...
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        ciphertext_multi = sc_multi_alice.encrypt(plainblob, nonce_multi);
        nonce_multi.increment();
    }
    auto t01 = system_clock::now();
    auto tmulti = duration_cast<milliseconds>(t01 - t00).count();

    os << "Encrypting " << nr_of_messages << " messages (multi ): " << tmulti
    ...
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        ciphertext_single =
          sc_single_alice.encrypt(plainblob, keypair_alice, nonce_single);
        nonce_single.increment();
    }
    auto t11 = system_clock::now();
    auto tsingle = duration_cast<milliseconds>(t11 - t10).count();

    os << "Encrypting " << nr_of_messages << " messages (single): " << tsingle
       << " milliseconds." << std::endl;
...
time_encrypt_detached(const unsigned long nr_of_messages)
{
    ...
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        ciphertext_multi =
          sc_multi_alice.encrypt(plainblob, nonce_multi, mac_multi);
        nonce_multi.increment();
    }
    auto t01 = system_clock::now();
    auto tmulti = duration_cast<milliseconds>(t01 - t00).count();
    ...
    auto t10 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        ciphertext_single = sc_single_alice.encrypt(
          plainblob, keypair_alice, nonce_single, mac_single);
        nonce_single.increment();
    }
    auto t11 = system_clock::now();
    auto tsingle = duration_cast<milliseconds>(t11 - t10).count();

    os << "Encrypting(detached) " << nr_of_messages
       << " messages (single): " << tsingle << " milliseconds." << std::endl;
...
time_decrypt(const unsigned long nr_of_messages)
{
    ...
    auto t00 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        decrypted_multi = sc_multi_alice.decrypt(ciphertext_multi, nonce_multi);
    ...
    os << "Decrypting " << nr_of_messages << " messages (multi ): " << tmulti
       << " milliseconds." << std::endl;

    // 2. time decrypting nr_of_messages with box
    auto t10 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        decrypted_single = sc_single_alice.decrypt(
          ciphertext_single, keypair_alice, nonce_single);
    ...
    os << "Decrypting " << nr_of_messages << " messages (single): " << tsingle
       << " milliseconds." << std::endl;
...
time_decrypt_detached(const unsigned long nr_of_messages)
{
    ...
    BT ciphertext_multi =
      sc_multi_alice.encrypt(plainblob, nonce_multi, mac_multi);
    BT ciphertext_single = sc_single_alice.encrypt(
      plainblob, keypair_alice, nonce_single, mac_single);
    ...
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        decrypted_multi =
          sc_multi_alice.decrypt(ciphertext_multi, nonce_multi, mac_multi);
        // since we decrypt over and over the same ciphertext message,
        // we don't nonce_multi.increment() here.
    }
    auto t01 = system_clock::now();
    ...
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        decrypted_single = sc_single_alice.decrypt(
          ciphertext_single, keypair_alice, nonce_single, mac_single);
        // since we decrypt over and over the same ciphtertext message,
        // we don't nonce_single.increment() here.
    }
    auto t11 = system_clock::now();
    ...
    os << "Decrypting(detached) " << nr_of_messages
       << " messages (single): " << tsingle << " milliseconds." << std::endl;
...
BOOST_AUTO_TEST_CASE(sodium_precomputed_test_encrypt_to_self_bytes)
{
    ...
    BOOST_TEST(ciphertext.size() ==
               plainblob.size() + box_precomputed<>::MACSIZE);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_fulltext_bytes)
{
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_empty_text_bytes)
{
    std::string plaintext{};
...
BOOST_AUTO_TEST_CASE(sodium_box_precomputed_test_destroysharedkey_decrypt_bytes)
{
    ...
    BOOST_TEST(
      destroy_shared_key_then_decrypt(ciphertext, keypair_alice, nonce));
}

BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_destroysharedkey_decrypt_bytes_detached)
{
    keypair<> keypair_alice;
    typename box_precomputed<>::nonce_type nonce;
    box_precomputed<> sc_alice(keypair_alice);
    ...
    BOOST_TEST(destroy_shared_key_then_decrypt_detached(
      ciphertext, keypair_alice, nonce, mac));
}

BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_encrypt_bytes)
{
    time_encrypt<>(1000);
    time_encrypt<>(10000);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_decrypt_bytes)
{
    time_decrypt<>(1000);
    time_decrypt<>(10000);
...
BOOST_AUTO_TEST_CASE(sodium_box_precomputed_test_full_plaintext_bytes_protected)
{
    ...
    BOOST_TEST(
      test_of_correctness_detached<sodium::bytes_protected>(plaintext));
}

BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_empty_plaintext_bytes_protected)
{
    std::string plaintext{};
    BOOST_TEST(test_of_correctness<sodium::bytes_protected>(plaintext));
    BOOST_TEST(
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_encrypt_to_self_bytes_protected)
{
    ...
    BOOST_TEST(ciphertext.size() ==
               plainblob.size() +
                 box_precomputed<sodium::bytes_protected>::MACSIZE);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_encrypt_to_self_bytes_protected_detached)
{
    keypair<sodium::bytes_protected> keypair_alice;
    typename box_precomputed<sodium::bytes_protected>::nonce_type nonce;
    box_precomputed<sodium::bytes_protected> sc_alice(keypair_alice);
    ...
    sodium::bytes_protected mac(
      box_precomputed<sodium::bytes_protected>::MACSIZE);

    sodium::bytes_protected ciphertext =
      sc_alice.encrypt(plainblob, nonce, mac);

    BOOST_TEST(ciphertext.size() == plainblob.size());
    BOOST_TEST(mac.size() == box_precomputed<sodium::bytes_protected>::MACSIZE);

    sodium::bytes_protected decrypted =
      sc_alice.decrypt(ciphertext, nonce, mac);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_fulltext_bytes_protected)
{
    std::string plaintext{ "Hi Alice, this is Bob!" };
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_empty_text_bytes_protected)
{
    std::string plaintext{};
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_falsify_ciphertext_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_falsify_mac_fulltext_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_falsify_mac_empty_bytes_protected)
{
    std::string plaintext{};
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_destroysharedkey_encrypt_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    sodium::bytes_protected plainblob{ plaintext.cbegin(), plaintext.cend() };

    BOOST_TEST(
      destroy_shared_key_then_encrypt<sodium::bytes_protected>(plainblob));
    BOOST_TEST(
      destroy_shared_key_then_encrypt_detached<sodium::bytes_protected>(
        plainblob));
}

BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_destroysharedkey_decrypt_bytes_protected)
{
    keypair<sodium::bytes_protected> keypair_alice;
    typename box_precomputed<sodium::bytes_protected>::nonce_type nonce;
    box_precomputed<sodium::bytes_protected> sc_alice(keypair_alice);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_destroysharedkey_decrypt_bytes_protected_detached)
{
    keypair<sodium::bytes_protected> keypair_alice;
    typename box_precomputed<sodium::bytes_protected>::nonce_type nonce;
    box_precomputed<sodium::bytes_protected> sc_alice(keypair_alice);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_encrypt_bytes_protected)
{
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_decrypt_bytes_protected)
{
...
BOOST_AUTO_TEST_CASE(sodium_box_precomputed_test_encrypt_to_self_chars)
{
    ...
    BOOST_TEST(ciphertext.size() ==
               plainblob.size() + box_precomputed<sodium::chars>::MACSIZE);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_fulltext_chars)
{
    std::string plaintext{ "Hi Alice, this is Bob!" };
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_detect_wrong_sender_empty_text_chars)
{
    std::string plaintext{};
...
BOOST_AUTO_TEST_CASE(sodium_box_precomputed_test_destroysharedkey_encrypt_chars)
{
    ...
    BOOST_TEST(
      destroy_shared_key_then_encrypt_detached<sodium::chars>(plainblob));
}
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_destroysharedkey_decrypt_chars_detached)
{
    keypair<sodium::chars> keypair_alice;
    typename box_precomputed<sodium::chars>::nonce_type nonce;
    box_precomputed<sodium::chars> sc_alice(keypair_alice);
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_encrypt_chars)
{
...
BOOST_AUTO_TEST_CASE(
  sodium_box_precomputed_test_time_multimessages_decrypt_chars)
{
