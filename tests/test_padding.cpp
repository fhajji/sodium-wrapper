...
BOOST_AUTO_TEST_CASE(sodium_test_padding_pad_full_already_blocksize)
{
    ...
    BOOST_CHECK(padded.size() ==
                16 + 16); // original size + one full additional block
}
...
BOOST_AUTO_TEST_CASE(sodium_test_padding_unpad_tamper_with_pad)
{
    ...
    try {
        ...
        BOOST_TEST_MESSAGE(
          "sodium::unpad() detected pad tampering, as expected");
    }
}
...
BOOST_AUTO_TEST_CASE(sodium_test_padding_unpad_inplace_tamper_with_pad)
{
    ...
    try {
        ...
        BOOST_TEST_MESSAGE(
          "sodium::unpad_inplace() detected pad tampering, as expected");
    }
}
