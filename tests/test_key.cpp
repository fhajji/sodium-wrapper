...
void
selectKey(const sodium::key<KEYSIZE>& key)
{
...
template<std::size_t KEYSIZE>
void
selectKey(sodium::key<KEYSIZE>&& key)
{
    BOOST_TEST_MESSAGE("selectKey(key<KEYSIZE> &&) invoked");

    sodium::key<KEYSIZE> internalKey{ std::move(
      key) }; // pilfer resources from parameter
...
BOOST_AUTO_TEST_CASE(sodium_test_key_move_ctor)
{
    ...
    sodium::key<ks1> key_move{ std::move(
      key) }; // move c'tor, key is now invalid
