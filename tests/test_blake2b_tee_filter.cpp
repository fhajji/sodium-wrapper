...
using blake2b_to_vector_null_type =
  blake2b_tee_device<io::null_sink, vector_sink>;
                       ...
                       const std::string& outfile_name)
{
    ...
    if (key.size() != 0) {
        blake2b_to_file_type blake2b_filter(
          blake2bfile, key, blake2b_to_file_type::HASHSIZE);
                       ...
                       const std::string& outfile_name)
{
    ...
    if (key.size() != 0) {
        blake2b_to_vector_type blake2b_to_vector_output_device(
          outfile,      // Device
          blake2b_sink, // Sink
          key,
  ...
  const typename blake2b_to_vector_null_type::key_type& key)
{
    ...
    if (key.size() != 0) {
        blake2b_to_vector_null_type blake2b_to_vector_null_output_device(
...
BOOST_AUTO_TEST_CASE(sodium_test_blake2b_filter_blake2b_to_file)
{
    ...
    pipeline_output_device(plaintext, key, hashfile_name, outfile_name);

    auto result = verify_hash(plaintext, key, hashfile_name, outfile_name);
...
BOOST_AUTO_TEST_CASE(sodium_test_blake2b_filter_blake2b_to_vector)
{
    ...
    hash_array_type hash{ pipeline_output_device(
      plaintext, key, outfile_name) };

    auto result = verify_hash(plaintext, key, hash, outfile_name);
...
BOOST_AUTO_TEST_CASE(sodium_test_blake2b_filter_blake2b_to_vector_null)
{
    ...
    blake2b_to_vector_null_type::key_type key(
      blake2b_to_vector_null_type::HASHSIZE);

    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };

    hash_array_type hash{ pipeline_output_device(plaintext, key) };

    auto result = verify_hash(plaintext, key, hash);
