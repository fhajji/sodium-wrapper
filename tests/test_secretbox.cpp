                    ...
                    bool falsify_nonce = false)
{
    ...
    try {
        BT decrypted = (falsify_key ? sc2 : sc)
                         .decrypt(ciphertext, (falsify_nonce ? nonce2 : nonce));

        BOOST_TEST(decrypted.size() == plainblob.size());

        // decryption succeeded and plainblob == decrypted if and only if
        // we didn't falsify the ciphertext nor the MAC nor the key nor the
        // nonce

        return !falsify_ciphertext && !falsify_mac && !falsify_key &&
               !falsify_nonce && (plainblob == decrypted);
                            ...
                            bool falsify_nonce = false)
{
    ...
    try {
        ...
        if (falsify_key)
            sc2.decrypt(decrypted, ciphertext, falsify_nonce ? nonce2 : nonce);
        else
            sc.decrypt(decrypted, ciphertext, falsify_nonce ? nonce2 : nonce);
                             ...
                             bool falsify_nonce = false)
{
    ...
    try {
        BT decrypted =
          (falsify_key ? sc2 : sc)
            .decrypt(ciphertext, (falsify_nonce ? nonce2 : nonce), mac);
                                     ...
                                     bool falsify_nonce = false)
{
    ...
    try {
        ...
        if (falsify_key)
            sc2.decrypt(
              decrypted, ciphertext, falsify_nonce ? nonce2 : nonce, mac);
        else
            sc.decrypt(
              decrypted, ciphertext, falsify_nonce ? nonce2 : nonce, mac);
...
time_decrypt(const unsigned long nr_of_messages = TIMING_RUNS_DEFAULT)
{
    ...
    auto t10 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        sc.decrypt(decrypted_inplace, ciphertext_with_mac, nonce);
    }
    auto t11 = system_clock::now();
    auto t_inplace = duration_cast<milliseconds>(t11 - t10).count();
    ...
    BOOST_CHECK_MESSAGE(t_inplace < t_no_inplace,
                        "sodium::secretbox::decrypt(inplace) slower than "
                        "sodium::box::decrypt() ???");
}
...
time_encrypt_detached(const unsigned long nr_of_messages = TIMING_RUNS_DEFAULT)
{
    ...
    auto t10 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        sc.encrypt(ciphertext_inplace, plainblob, nonce, mac);
    }
    auto t11 = system_clock::now();
    auto t_inplace = duration_cast<milliseconds>(t11 - t10).count();
    ...
    BOOST_CHECK_MESSAGE(t_inplace < t_no_inplace,
                        "sodium::secretbox::encrypt(inplace) detached slower "
                        "than sodium::box::encrypt() ???");
}
...
time_decrypt_detached(const unsigned long nr_of_messages = TIMING_RUNS_DEFAULT)
{
    ...
    auto t10 = system_clock::now();
    for (unsigned long i = 0; i != nr_of_messages; ++i) {
        sc.decrypt(decrypted_inplace, ciphertext, nonce, mac);
    }
    auto t11 = system_clock::now();
    auto t_inplace = duration_cast<milliseconds>(t11 - t10).count();
    ...
    BOOST_CHECK_MESSAGE(t_inplace < t_no_inplace,
                        "sodium::secretbox::decrypt(inplace) detached slower "
                        "than sodium::box::decrypt() ???");
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_key_bytes)
{
    ...
    BOOST_TEST(
      test_of_correctness_inplace<>(plaintext, false, false, true, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_nonce_bytes)
{
    ...
    BOOST_TEST(
      test_of_correctness_inplace<>(plaintext, false, false, false, true));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_ciphertext_and_mac_bytes)
{
    ...
    BOOST_TEST(
      test_of_correctness_inplace<>(plaintext, true, true, false, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_ciphertext_detached_bytes)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<>(plaintext, true, false, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<>(
      plaintext, true, false, false, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_mac_detached_bytes)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<>(plaintext, false, true, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<>(
      plaintext, false, true, false, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_key_detached_bytes)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<>(plaintext, false, false, true, false));
    BOOST_TEST(test_of_correctness_detached_inplace<>(
      plaintext, false, false, true, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_nonce_detached_bytes)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<>(plaintext, false, false, false, true));
    BOOST_TEST(test_of_correctness_detached_inplace<>(
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_ciphertext_and_mac_detached_bytes)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<>(plaintext, true, true, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<>(
      plaintext, true, true, false, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_key_chars)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness<chars>(plaintext, false, false, true, false));
    BOOST_TEST(
      test_of_correctness_inplace<chars>(plaintext, false, false, true, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_nonce_chars)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness<chars>(plaintext, false, false, false, true));
    BOOST_TEST(
      test_of_correctness_inplace<chars>(plaintext, false, false, false, true));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_ciphertext_and_mac_chars)
{
    ...
    BOOST_TEST(
      test_of_correctness_inplace<chars>(plaintext, true, true, false, false));
}

BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_ciphertext_detached_chars)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness_detached<chars>(
      plaintext, true, false, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<chars>(
      plaintext, true, false, false, false));
}
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_ciphertext_and_mac_detached_chars)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(
      test_of_correctness_detached<chars>(plaintext, true, true, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<chars>(
      plaintext, true, true, false, false));
}
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_full_plaintext_detached_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(plaintext));
    BOOST_TEST(
      test_of_correctness_detached_inplace<bytes_protected>(plaintext));
}

BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_empty_plaintext_detached_bytes_protected)
{
    std::string plaintext{};
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(plaintext));
    BOOST_TEST(
...
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness<bytes_protected>(
      plaintext, false, false, true, false));
    BOOST_TEST(test_of_correctness_inplace<bytes_protected>(
      plaintext, false, false, true, false));
}
...
BOOST_AUTO_TEST_CASE(sodium_secretbox_test_falsify_mac_empty_bytes_protected)
{
    ...
    BOOST_TEST(
      test_of_correctness_inplace<bytes_protected>(plaintext, false, true));
}

BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_ciphertext_and_mac_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness<bytes_protected>(
      plaintext, true, true, false, false));
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_ciphertext_detached_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, true, false, false, false));
    BOOST_TEST(test_of_correctness_detached_inplace<bytes_protected>(
      plaintext, true, false, false, false));
}
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_nonce_detached_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, false, false, false, true));
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_mac_empty_detached_bytes_protected)
{
    std::string plaintext{};
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, false, true, false, false));
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_key_empty_detached_bytes_protected)
{
    std::string plaintext{};
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, false, false, true, false));
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_nonce_empty_detached_bytes_protected)
{
    std::string plaintext{};
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, false, false, false, true));
...
BOOST_AUTO_TEST_CASE(
  sodium_secretbox_test_falsify_ciphertext_and_mac_detached_bytes_protected)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    BOOST_TEST(test_of_correctness_detached<bytes_protected>(
      plaintext, true, true, false, false));
