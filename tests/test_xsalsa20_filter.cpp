// test_xsalsa20_filter.cpp -- Test sodium::xsalsa20_filter
//
// ISC License
//
...
#define BOOST_TEST_MODULE sodium::xsalsa20_filter Test
...
test_of_correctness_combined_output_filter(const std::string& plaintext)
{
    ...
    xsalsa20_filter::key_type key;     // Create a random key
    xsalsa20_filter::nonce_type nonce; // Create a random nonce

    xsalsa20_filter encrypt_filter{ 10, key, nonce };
    xsalsa20_filter decrypt_filter{ 12, key, nonce };
                                  ...
                                  bool falsify_nonce = false)
{
    ...
    xsalsa20_filter::key_type key;      // Create a random key
    xsalsa20_filter::key_type key2;     // Create another random key
    xsalsa20_filter::nonce_type nonce;  // Create a random nonce
    xsalsa20_filter::nonce_type nonce2; // Create another random nonce

    xsalsa20_filter encrypt_filter{ 10, key, nonce };
    xsalsa20_filter decrypt_filter{ 12,
    ...
    chars plainblob2{ plainblob };
    std::copy(
      plainblob.cbegin(), plainblob.cend(), std::back_inserter(plainblob2));
                       ...
                       const std::string& decfile_name)
{
    ...
    io::filtering_ostream os(encrypt_filter | tee_filter | decrypt_filter |
                             decfile);
...
BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_size_full_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    length_test_output_filter(plaintext);
}

BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_size_empty_output_filter)
{
    std::string plaintext{};
    length_test_output_filter(plaintext);
}

BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_size_full_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    length_test_input_filter(plaintext);
}

BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_size_empty_input_filter)
{
    std::string plaintext{};
    length_test_input_filter(plaintext);
}
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_combined_full_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_combined_empty_output_filter)
{
    std::string plaintext{};
    auto result = test_of_correctness_combined_output_filter(plaintext);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_combined_full_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_combined_empty_input_filter)
{
    std::string plaintext{};
    auto result = test_of_correctness_combined_input_filter(plaintext);
...
BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_correctness_full_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_ciphertext_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_output_filter(plaintext, true, false, false);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_key_full_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_output_filter(plaintext, false, true, false);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_nonce_full_output_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_output_filter(plaintext, false, false, true);

    // Test must succeed, we falsified the nonce but caught it!
    BOOST_CHECK(result);
}

BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_correctness_full_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_ciphertext_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_input_filter(plaintext, true, false, false);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_key_full_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_input_filter(plaintext, false, true, false);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_xsalsa20_filter_correctness_falsify_nonce_full_input_filter)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    auto result =
      test_of_correctness_input_filter(plaintext, false, false, true);

    // Test must succeed, we falsified the nonce but caught it!
    BOOST_CHECK(result);
}

BOOST_AUTO_TEST_CASE(sodium_test_xsalsa20_filter_pipeline_output_device)
{
    std::string plaintext{ "the quick brown fox jumps over the lazy dog" };
    pipeline_output_device(plaintext, "outfile4.enc", "outfile4.dec");

    // Test succeeds if outfile.dec contains plaintext
    chars plainblob{ plaintext.cbegin(), plaintext.cend() };

    io::file_source is("outfile4.dec",
    ...
    delete_file("outfile4.enc");
    delete_file("outfile4.dec");
}

// NYI: add test cases to prove that we used xsalsa20 correctly...
// i.e. compare with crypto_stream_xsalsa20_xor() on the whole input.
