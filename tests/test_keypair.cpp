...
{
    keypair<> keypair1{};
    keypair<> keypair2(keypair1.private_key().data(),
...
BOOST_AUTO_TEST_CASE(sodium_test_keypair_copy_ctor_bytes)
{
    ...
    BOOST_TEST(
      (keypair1 == keypair_copy)); // check also operator==() on keypair(s)

    BOOST_TEST((keypair1.private_key() ==
                keypair_copy.private_key())); // key::operator==()
    BOOST_TEST(keypair1.public_key() ==
...
BOOST_AUTO_TEST_CASE(sodium_test_keypair_seedcompare_ctor_different_seed_bytes)
{
    ...
    BOOST_TEST(
      !sodium::compare(seed1, seed2)); // very unlikely that they are the same
...
BOOST_AUTO_TEST_CASE(sodium_test_keypair_size_ctor_privkey_bytes_protected)
{
    ...
    keypair<bytes_protected> keypair2(keypair1.private_key().data(),
                                      keypair1.private_key().size());
...
BOOST_AUTO_TEST_CASE(
  sodium_test_keypair_seedcompare_ctor_same_seed_bytes_protected)
{
    bytes_protected seed(ks_seed);
    sodium::randombytes_buf_inplace(seed);
...
BOOST_AUTO_TEST_CASE(
  sodium_test_keypair_seedcompare_ctor_different_seed_bytes_protected)
{
    bytes_protected seed1{ sodium::randombytes_buf<bytes_protected>(ks_seed) };
    bytes_protected seed2{ sodium::randombytes_buf<bytes_protected>(ks_seed) };
...
BOOST_AUTO_TEST_CASE(sodium_test_keypair_size_ctor_privkey_chars)
{
    ...
    keypair<chars> keypair2(keypair1.private_key().data(),
                            keypair1.private_key().size());
